...
namespace WebApi.Repositories
{
    public class MaintenanceRepository : IMaintenanceRepository
    {
        ...
        public async Task<MaintenanceContractorLine[]> GetAllMaintenanceContractorLineAsync(int MaintenanceID)
        {
            IQueryable<MaintenanceContractorLine> query = _appDbContext.MaintenanceContractorLines
                .Where(x => x.MaintenaceID == MaintenanceID);
            return await query.ToArrayAsync();
        }
        public async Task<MaintenanceContractorLine> GetMaintenanceContractorLineByID(int MaintenanceID,int contractorId)
        {
            IQueryable<MaintenanceContractorLine> query = _appDbContext.MaintenanceContractorLines
                .Where(x => x.MaintenaceID == MaintenanceID&&x.ContractorID==contractorId);
            return await query.FirstOrDefaultAsync();
        }
        public async Task<MaintenanceContractorLine> DeleteAllMaintenanceContractorLineAsync(MaintenanceContractorLine[] item)
        {
            for (int i = 0; i < item.Length; i++)
            {
                _appDbContext.MaintenanceContractorLines.Remove(item.ElementAt(i));
                await _appDbContext.SaveChangesAsync();
            }
