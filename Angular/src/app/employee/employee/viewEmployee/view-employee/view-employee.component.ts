...
export class ViewEmployeeComponent {
  ...
  employee: Employee [] = [
    {id: 1, email: 'johndoe@propco.co.za', name: 'John', surname: 'Doe', phone: '0798903590', jobTitle: 'Administator'},
    {id: 2, email: 'brandon@propco.co.za', name: 'Brandon', surname: 'Driver', phone: '0834477955', jobTitle: 'Asset Manager'},
    {id: 3, email: 'myles@propco.co.za', name: 'Myles', surname: 'Brown', phone: '0824479855', jobTitle: 'Financial Manager'},
  ];
  ...
  openUpdateEmployeeModal(employee: Employee): void {
    ...
    dialogRef.componentInstance.employeeUpdated.subscribe((updatedEmployee: Employee) => {
      const index = this.employee.findIndex(u => u.id === updatedEmployee.id);
      if (index !== -1) {
        this.employee[index] = updatedEmployee;
        this.filtered = this.employee.filter(u => u.name.toLowerCase().includes(this.searchTerm.toLowerCase()));
      }
    });
  }
